...
describe('When logged in', () => {
	...
	describe('And using valid inputs', () => {
		...
		beforeEach(async () => {
			await superPage.type('input[name="title"]', titleText);
			await superPage.type('input[name="content"]', contentText);
			await superPage.click('form button');
		});
		test('takes user to review screen', async () => {
			const text = await superPage.getContentsOf('form h5');
			expect(text).toEqual('Please confirm your entries');
		});
		test('submitting then saving adds blog to index page', async () => {
			await superPage.click('button.green');
			await superPage.waitFor('div.card');
